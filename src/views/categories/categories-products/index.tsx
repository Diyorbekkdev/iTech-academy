...
export const CategoriesProductsPage = ({ lang }: ICategoriesPage) => {
  const { id } = useParams();
  ...
  return (
    <section className="container">
      <Breadcrumbs variant="solid">
        ...
        <BreadcrumbItem href="/categories">Mahsulotlar kategoriyasi</BreadcrumbItem>
        <BreadcrumbItem
          href="#"
          className="font-semibold"
        >
          {data?.data?.[0]?.name?.[lang]}
        </BreadcrumbItem>
      </Breadcrumbs>
      ...
      <div className="mt-6 text-xl">
        {isLoading ? (
          ...
          <h1 className="font-bold mb-6">{data?.data?.[0]?.name?.[lang]}</h1>
        )}
        <Divider />
      </div>
        ...
        <div className="flex-1">
          ...
          <div className="mt-6 sm:border-l-1 sm:pl-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
            {isLoading
              ...
              : data?.data?.[0]?.products?.map((el: IProduct) => (
                  <ProductCard
                    key={el?._id}
                    {...el}
                    lang={lang}
